...
class ImageFeatExtract(nn.Module):
    def __init__(self):
        ...
        self.c1 = nn.Conv2d(1024, 640, 1)               # 640
        self.n1 = nn.BatchNorm2d(640, affine=True)      # 640

    def forward(self, image, template_feat):

        x0 = self.backdense_0(image)
        x0_t = self.conv2d_dw_group(x0, template_feat, padding=2)
        x0_cat = x0 + x0_t
...
class TemplateFeatExtractGlobal(nn.Module):
    def __init__(self, output_dim=1024):
        ...
        self.final_pool_1 = nn.MaxPool2d(2, 2)
        self.final_pool_2 = nn.MaxPool2d(2, 2)
    ...
    def forward(self, img):
        ...
        x1_down = F.interpolate(x1_norm, size=kernel_size, mode='bilinear', align_corners=True)
        x2_down = F.interpolate(x2_norm, size=kernel_size, mode='bilinear', align_corners=True)
...
#        xf = self.final_norm_2(F.elu(self.final_conv_2(xf)))
        xf = self.final_norm_1(self.final_pool_1(F.elu(self.final_conv_1(xf))))
        xf = self.final_norm_2(self.final_pool_2(F.elu(self.final_conv_2(xf))))
...
class TemplateFeatExtract(nn.Module):
    def __init__(self, output_dim=1024):
        ...
        self.backbone_0 = torch.nn.Sequential(*modules[0:1])
        self.backbone_1 = torch.nn.Sequential(*modules[1:5])
        self.backbone_2 = torch.nn.Sequential(*modules[5:8])
        self.backbone_3 = torch.nn.Sequential(*modules[8:])

        self.norm_1 = nn.BatchNorm2d(128, affine=True)
        self.norm_2 = nn.BatchNorm2d(256, affine=True)
        self.norm_3 = nn.BatchNorm2d(512, affine=True)

        self.final_conv_1 = nn.Conv2d(896, 720, 3, padding=1)
        self.final_conv_2 = nn.Conv2d(720, 640, 3, padding=1)

        self.final_pool_1 = nn.MaxPool2d(2, 2)
        self.final_pool_2 = nn.MaxPool2d(2, 2)

        self.final_norm_1 = nn.BatchNorm2d(720, affine=True)
        self.final_norm_2 = nn.BatchNorm2d(640, affine=True)
    ...
    def forward(self, img):
        ...
        x1_norm = self.norm_1(x1)
        x2_norm = self.norm_2(x2)
        x3_norm = self.norm_3(x3)

        kernel_size = x3.size(3)
        x1_down = F.interpolate(x1_norm, size=kernel_size, mode='bilinear', align_corners=True)
        x2_down = F.interpolate(x2_norm, size=kernel_size, mode='bilinear', align_corners=True)
        x3_down = F.interpolate(x3_norm, size=kernel_size, mode='bilinear', align_corners=True)

        xf = torch.cat([x3_down, x2_down, x1_down], dim=1)
#        xf = self.final_norm_1(self.final_pool_1(xf))
#        xf = self.final_norm_2(self.final_pool_2(xf))
        xf = self.final_norm_1(self.final_pool_1(F.elu(self.final_conv_1(xf))))
        xf = self.final_norm_2(self.final_pool_2(F.elu(self.final_conv_2(xf))))
...
class CorrelationModel(nn.Module):
    def __init__(self, input_dim=1024):
        ...
        self.corr_conv_dot5x5 = nn.Conv2d(input_dim, 256, 3, padding=1)
        self.norm_corr_dot5x5 = nn.BatchNorm2d(256, affine=True)

        self.corr_conv_sub = nn.Conv2d(input_dim, 256, 3, padding=1)
        self.norm_corr_sub = nn.BatchNorm2d(256, affine=True)

        self.cf = nn.Conv2d(1024, 512, 3, padding=1)
#        self.cf = nn.Conv2d(768, 512, 3, padding=1)
    ...
    def forward(self, image_feat, template_feat, test=False):
        ...
        dot5x5 = self.conv2d_dw_group(image_feat, t1, padding=2)
        ...
        dot_conv5x5 = self.norm_corr_dot5x5(F.elu(self.corr_conv_dot5x5(dot5x5)))
        sub_conv = self.norm_corr_sub(F.elu(self.corr_conv_sub(sub)))

        #outputs
        x = torch.cat([dot_conv, sub_conv, dot_conv3x3, dot_conv5x5], dim=1)
        ...
        s4 = F.interpolate(self.ns4(F.elu(self.s4(s3))), size=IMG_SIZE, mode="nearest")
...
class Network(NetworkBase):
    def __init__(self):
        ...
        self.correlation_model = CorrelationModel(640)      # 896
    ...
    def forward(self, img, tmp):
        ...
        template_features_global = self.template_feature_extractor_global(tmp)
        features = self.image_feature_extractor(img, template_features_global)
        template_features = self.template_feature_extractor(tmp)
...
if __name__ == '__main__':
    ...
    tmp = torch.zeros(1, 3, 640, 480)

    tmp_feat_extractor_glob = TemplateFeatExtractGlobal()
    tmp_feat_extractor = TemplateFeatExtract()
    img_feat_extractor = ImageFeatExtract()

    template_features_global = tmp_feat_extractor_glob(tmp)
    image_feat = img_feat_extractor(img, template_features_global)
    template_feat = tmp_feat_extractor(tmp)

    # print(image_feat.size())
    # print(template_feat.size())

    model = CorrelationModel(640)

    t1 = model.n1(F.elu(model.c1(template_feat)))
    t2 = model.n2(F.elu(model.c2(t1)))
    print(t1.size())
    print(t2.size())
    ...
    dot5x5 = model.conv2d_dw_group(image_feat, t1, padding=2)
    dot3x3 = model.conv2d_dw_group(image_feat, t2, padding=1)
    print(dot5x5.size())
    print(dot3x3.size())
    ...
    dot_conv = model.norm_corr_dot(F.elu(model.corr_conv_dot(dot)))
    dot_conv5x5 = model.norm_corr_dot5x5(F.elu(model.corr_conv_dot5x5(dot5x5)))
    dot_conv3x3 = model.norm_corr_dot3x3(F.elu(model.corr_conv_dot3x3(dot3x3)))
    sub_conv = model.norm_corr_sub(F.elu(model.corr_conv_sub(sub)))
    print(dot_conv.size())
    print(dot_conv5x5.size())
    print(dot_conv3x3.size())
    print(sub_conv.size())

    #outputs
    x = torch.cat([dot_conv, sub_conv, dot_conv3x3, dot_conv5x5], dim=1)
    x2 = model.nf(F.elu(model.cf(x)))
    print(x.size())
    print(x2.size())

    # model = TemplateFeatExtract()

    # x0 = model.backbone_0(tmp)

    # x1 = model.backbone_1(x0)
    # x2 = model.backbone_2(x1)
    # x3 = model.backbone_3(x2)

    # x1_norm = model.norm_1(x1)
    # x2_norm = model.norm_2(x2)
    # x3_norm = model.norm_3(x3)

    # kernel_size = x3.size(3)
    # x1_down = F.upsample(x1_norm, size=kernel_size, mode='bilinear')
    # x2_down = F.upsample(x2_norm, size=kernel_size, mode='bilinear')

    # xf = torch.cat([x3_norm, x2_down, x1_down], dim=1)
